...
import { string, funcName, boolean, link, channel, user, role } from './funcTypes';
...
describe('Function Type Parsing', () => {
	it('should parse function names correctly', () => {
		...
		checkCombinator({
			res: res1,
			expectedRes: { type: Type.Function, value: ['This', 'IS', 'a', 'Function', 'name'] }
		});
		checkCombinator({ res: res2, expectedRes: { type: Type.Function, value: ['invalid'] } });
	});
